...
public class FormatosActas {
	...
	public FormatosActas(Context context, String FolderAplicacion, boolean _copiaSistema){
		...
		ImpSQL 	= new SQLite(this.context, this._folderAplicacion);
		DT 		= new DateTime();
		FcnZebra= new zebra_QL420(600, 40, 10, 10, 100, _copiaSistema);
	}
	...
	public void FormatoVerificacion(String idSerial, String cuentaCliente, int copiaImpresion){	
		...
		FcnZebra.DrawImage("ENERCA.PCX", 30, 25);
		FcnZebra.WrTitulo("ACTA DE SUSPENSION, CORTE Y RECONEXION", 0, 3);
		 
		 
		 
		/********************************Validaciones necesarias para el encabezado del acta**************************************/
		FcnZebra.WrLabel("Acta No.  ","xxxxxx", 200, 0, 1.2);
		...
		this._infRegistro1 = ImpSQL.SelectDataRegistro(	"amd_ordenes_trabajo", 
														...
														"dig_observacion, dig_observacion_sellos, dig_nombre_usuario, dig_ident_usuario, dig_tel_usuario", 
														"id_serial="+idSerial+" AND cuenta="+cuentaCliente);		 
		
		/**Datos inicio visita**/
		FcnZebra.WrLabel("Fecha: ", this._infRegistro1.getAsString("dig_fecha_visita"), 10, 0, 0);
		FcnZebra.WrLabel("Hora: ", this._infRegistro1.getAsString("dig_hora_visita"), 200, 0, 0);
		...
		FcnZebra.WrLabel("Ciclo:   	", this._infRegistro1.getAsString("ciclo"), 200, 0, 0);
		FcnZebra.WrLabel("Medidor No:", this._infRegistro1.getAsString("serie"), 400, 0, 1);
		...
		FcnZebra.WrLabel("Municipio:", this._infRegistro1.getAsString("municipio"), 10, 0, 3);
		
		FcnZebra.JustInformation("ESTIMADO CLIENTE: De conformidad en lo establecido en la ley 142 de 1994 y en el contrato de prestacion de servicio publicos con condiciones uniformes vigentes, personal autorizado por ENERCA S.A. E.S.P. procedio a:", 10, 2, 1);
		FcnZebra.WrLabel("Cod. Accion:", this._infRegistro1.getAsString("dig_codigo_accion"), 10, 0, 1);
		...
		if(this._infRegistro1.getAsBoolean("dig_material_retirado")){
			FcnZebra.WrLabel("Se retira material","Si(x)   No( )", 10, 0, 0);
		}else{
			FcnZebra.WrLabel("Se retira material","Si( )   No(x)", 10, 0, 0);
		}
		FcnZebra.WrLabel("No Canuelas",this._infRegistro1.getAsString("dig_num_canuelas"), 400, 0, 1);
		
		
		/**Pin de corte**/
		if(this._infRegistro1.getAsBoolean("dig_pin_corte")){
			FcnZebra.WrLabel("Pin Corte","Si(x)   No( )", 10, 0, 0);
		}else{
			FcnZebra.WrLabel("Pin Corte","Si( )   No(x)", 10, 0, 0);
		}
		FcnZebra.WrLabel("Otros",this._infRegistro1.getAsString("dig_otros"), 400, 0, 1);
		...
		if(this._infRegistro1.getAsBoolean("dig_cancelada_factura")){
			FcnZebra.WrLabel("Factura Cancelada","Si(x)   No( )", 10, 0, 0);
			FcnZebra.WrLabel("Fecha Pago:",this._infRegistro1.getAsString("dig_fecha_pago_factura"), 400, 0, 1);
			FcnZebra.WrLabel("Monto Cancelado",this._infRegistro1.getAsString("dig_monto_factura"), 10, 0, 0);
			FcnZebra.WrLabel("Entidad:",this._infRegistro1.getAsString("dig_entidad_factura"), 400, 0, 1);
		}else{
			FcnZebra.WrLabel("Factura Cancelada","Si( )   No(x)", 10, 0, 0);
			FcnZebra.WrLabel("Fecha Pago:","--", 400, 0, 1);
			FcnZebra.WrLabel("Monto Cancelado","--", 10, 0, 0);
			FcnZebra.WrLabel("Entidad:","--", 400, 0, 1);
		}
		...
		FcnZebra.WrSubTitulo("ATENCION VISITA",10,1,1);
		...
		FcnZebra.WrLabel("Firma Usuario.", "", 10, 0, 3);		
		
		FcnZebra.WrRectangle(10, FcnZebra.getCurrentLine(), 600, FcnZebra.getCurrentLine()+5, 1, 0);
		FcnZebra.WrLabel(ImpSQL.StrSelectShieldWhere("db_parametros", "valor", "item='nombre_tecnico'"),"", 10, 0, 1);
		FcnZebra.WrLabel("CC.", ImpSQL.StrSelectShieldWhere("db_parametros", "valor", "item='NPDA'"), 10, 0, 1);
		FcnZebra.WrLabel("Firma Tecnico Electricista.", "", 10, 0, 3);	
